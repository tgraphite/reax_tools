...
std::string Molecule::info()
{
	...
	std::string s = fmt::format("Mol: {} Bonds:{} Angles:{} Dihedrals:{} hash:{}\n", formula, mol_bonds.size(), mol_angles.size(), mol_dihedrals.size(), hash);
...
void Molecule::update_formula()
{
	...
	{
		types_nums[mol_atom->type_name]++; // If key of map not exist, create key-value and value++, If exists, value++.
	}
	...
	for (auto& bond : mol_bonds)
	{
		...
		for (auto& neigh : bond->atom_i->bonded_atoms)
		{
			if (neigh == bond->atom_j)
				continue;
			auto tmp_angle = std::make_shared<Angle>(neigh, bond->atom_i, bond->atom_j);
		...
		for (auto& neigh : bond->atom_j->bonded_atoms)
		{
			if (neigh == bond->atom_i)
				continue;
			auto tmp_angle = std::make_shared<Angle>(bond->atom_i, bond->atom_j, neigh);
			this->insert(tmp_angle);
		}
	}
	...
	for (auto& angle : mol_angles)
	{
		...
		for (auto& neigh : angle->atom_k->bonded_atoms)
		{
			if ((neigh == angle->atom_j) or (neigh == angle->atom_i))
				continue;
			auto tmp_dihedral = std::make_shared<Dihedral>(angle->atom_i, angle->atom_j, angle->atom_k, neigh);
		...
		for (auto& neigh : angle->atom_i->bonded_atoms)
		{
			if ((neigh == angle->atom_j) or (neigh == angle->atom_k))
				continue;
			auto tmp_dihedral = std::make_shared<Dihedral>(neigh, angle->atom_i, angle->atom_j, angle->atom_k);
			this->insert(tmp_dihedral);
		}
	}
}