...
// 外部定义模板特化
template <> inline bool ArgParser::get_default_for_flag<bool>() const
{
    return false;
}

template <> inline std::string ArgParser::get_default_for_flag<std::string>() const
{
    return "false";
}

template <> inline std::string ArgParser::convert<std::string>(const std::string &value) const
{
    return value;
}
...
template <> inline std::vector<std::string> ArgParser::convert<std::vector<std::string>>(const std::string &value) const
{
    return split(value, ",");
}
...
int main(int argc, char *argv[])
{
    ...
    // 添加示例
    parser.add_example("reax_tools -f traj.lammpstrj -t C,H,O,N,S,F -r 1.2 -nt 4 -me C -rc --dump\n"
