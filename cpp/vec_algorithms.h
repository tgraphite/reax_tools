...
inline std::vector<float> subtract_vectors(const std::vector<float>& vec_a, const std::vector<float>& vec_b) {
    std::vector<float> vec(vec_a.size());
    if (!(vec_a.size() == vec_b.size())) {
        std::cerr << "Vector size not equal." << std::endl;
    }
...
inline std::vector<float> vector_scale(const std::vector<float>& vec_a, const float& k) {
    std::vector<float> vec(vec_a.size());
...
inline std::vector<float> operator*(const std::vector<float>& lhs, const float& rhs) { return vector_scale(lhs, rhs); }
...
inline float distance_sq(const std::vector<float>& vec_a, const std::vector<float>& vec_b) {
    float dist_sq = 0.0f;
...
inline float distance_sq_pbc(const std::vector<float>& vec_a, const std::vector<float>& vec_b,
