...
| `--no-track-reactions` | 禁用反应跟踪（默认开启） | `--no-track-reactions` |
...
| `reaction_flow_full.dot` | 反应流网络图（DOT 格式） |
| `reaction_flow_simplified.dot` | 简化版反应流网络图（Top 60） |
| `key_molecules_flow.md` | 关键分子反应流汇总表 |
| `reaction_track_events.csv` | 反应事件频率统计（ChemTrayzer-like） |
...
### 反应跟踪分析 (ChemTrayzer-like)
基于原子连接关系变化识别反应事件，统计反应频率并追踪分子生命周期
